...
function controls(){const r=current();$('export').disabled=!r||!valid||!r.reviewed;$('reviewed').checked=!!r?.reviewed;$('generate').disabled=busy||!configured;$('generate').textContent=busy?'Proposal in progress…':'Create proposal →';$('runningLink').hidden=!busy;}
function reviewUI(){
...
function drawHistory(){
  ...
  for(const row of rows){const card=document.createElement('article');card.className='proposal-card';const top=makeText('div','','proposal-card-top');top.append(makeText('span','▤','doc-icon'),makeText('span',row.reviewed?'Reviewed':'Needs review','status-pill'+(row.reviewed?' reviewed':'')));const heading=document.createElement('h2'),link=makeText('a',row.fields.company||'Untitled proposal');link.href='#/proposal/'+row.id;heading.append(link);const sub=makeText('p',row.origin==='recovered'?'Recovered from your earlier draft':row.origin==='imported'?'Imported proposal · 4 A4 pages':'Cold email marketing pilot · 4 A4 pages','subtitle');const metrics=makeText('div','','card-metrics');for(const [field,label]of [['leads','Leads'],['duration','Duration'],['fee','Pilot fee']]){const metric=document.createElement('div');metric.append(makeText('b',row.fields[field]||'—'),makeText('span',label));metrics.append(metric);}const footer=makeText('div','','card-footer');const time=makeText('time','Edited '+formatUpdated(row.updatedAt));time.dateTime=row.updatedAt||'';const open=makeText('a','Open proposal →');open.href=link.href;footer.append(time,open);card.append(top,heading,sub,metrics,footer);$('historyList').append(card);}
}
function loadEditor(){
...
function renderRoute(){
  ...
  $('routeLabel').textContent=label;document.title=(current()?.fields.company||label)+' · Bootprint';window.scrollTo(0,0);controls();
}
...
async function status(){const s=await api('status');authenticated=s.authenticated;configured=s.configured;$('loginPanel').hidden=authenticated;$('app').hidden=!authenticated;$('connection').textContent=configured?'Ready to create':'API key needed';if(authenticated)renderRoute();}
...
$('save').onclick=()=>{const record=current();if(!record)return;const url=URL.createObjectURL(new Blob([JSON.stringify(record,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=(record.fields.company||'proposal').replace(/[^a-zA-Z0-9_-]/g,'-')+'-draft.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};

...
export default async function handler(req,res){
 ...
 if(req.method==='GET'&&action==='status')return send(200,{authenticated:session(req),configured:!!process.env.OPENAI_API_KEY,model:process.env.OPENAI_MODEL||'gpt-5.4'});
 ...
 try{return send(200,await generate(body.transcript,body.notes));}catch(err){return send(err.status||502,{error:err.name==='TimeoutError'?'Generation timed out. Your draft is unchanged. Try shorter input.':err.message});}
}
